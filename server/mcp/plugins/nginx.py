...
def _optional_config_arg(
    context: ToolExecutionContext, arguments: dict[str, Any]
) -> list[str]:
    if not arguments.get("config_path"):
        return []
    config_path = context.services.host_ops.resolve_managed_path(
        str(arguments["config_path"]), roots=_nginx_roots(context)
    )
...
async def nginx_status(
    ...
    test_result = await context.services.host_ops.run_backend(
        "nginx", "-t", *_optional_config_arg(context, arguments), check=False
    )
    return {
...
async def nginx_control(
    ...
    if action == "start":
        ...
        result = await context.services.host_ops.run_backend(
            ...
            "-s",
            action,
            *_optional_config_arg(context, arguments),
            check=False,
        )
    if result.returncode != 0:
...
PLUGIN = PluginDefinition(
    manifest=PluginManifest(
        ...
        permissions=[
            PermissionDefinition(
                key="nginx.read",
                description="Читать статус Nginx и управляемые пути конфигурации.",
            ),
            PermissionDefinition(
                key="nginx.write",
                description="Проверять и управлять локальным процессом Nginx.",
            ),
        ],
