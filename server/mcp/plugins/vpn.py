...
async def _vpn_control_availability(services) -> RuntimeAvailability:
    if services.host_ops.is_linux:
        return services.host_ops.availability_for_any_command(
            ["wg-quick", "openvpn", "systemctl"], providers=["wireguard", "openvpn"]
        )
    return services.host_ops.availability_for_any_command(
        ["sc", "wireguard"], providers=["windows-service", "wireguard"]
    )
...
async def import_profile(
    ...
    source_path = context.services.host_ops.resolve_managed_path(
        str(require_argument(arguments, "source_path")),
    ...
    if source_path.suffix.lower() not in _ALLOWED_VPN_EXTS:
        raise RuntimeError(
            f"Invalid VPN profile extension. Allowed: {', '.join(_ALLOWED_VPN_EXTS)}"
        )
...
async def vpn_status(
    ...
    if context.services.host_ops.is_windows:
        result = await context.services.host_ops.run_backend(
            "sc", "query", service_name, check=False
        )
        return {
            "name": name,
    ...
    if vpn_type == "wireguard" and context.services.host_ops.command_exists("wg"):
        ...
        return {
            "name": name,
            "vpn_type": vpn_type,
            "running": service_name in result.stdout or name in result.stdout,
            **result.to_dict(),
        }
    if profile.get("service_name") and context.services.host_ops.command_exists(
        "systemctl"
    ):
        result = await context.services.host_ops.run_backend(
            "systemctl", "status", service_name, check=False
        )
        return {
            "name": name,
            "service_name": service_name,
...
async def vpn_control(
    ...
    if context.services.host_ops.is_windows:
        ...
        if action == "restart":
            stop_result = await context.services.host_ops.run_backend(
                "sc", "stop", service_name, check=False
            )
            start_result = await context.services.host_ops.run_backend(
                "sc", "start", service_name, check=False
            )
            return {
                "name": name,
                "action": action,
        ...
        result = await context.services.host_ops.run_backend(
            "sc", mapped, service_name, check=False
        )
        return {"name": name, "action": action, **result.to_dict()}
    if vpn_type == "wireguard" and context.services.host_ops.command_exists("wg-quick"):
        if action == "restart":
            down_result = await context.services.host_ops.run_backend(
                "wg-quick", "down", config_path, check=False
            )
            up_result = await context.services.host_ops.run_backend(
                "wg-quick", "up", config_path, check=False
            )
            return {
                "name": name,
                "action": action,
                "down": down_result.to_dict(),
                "up": up_result.to_dict(),
            }
        result = await context.services.host_ops.run_backend(
            "wg-quick", "up" if action == "start" else "down", config_path, check=False
        )
        return {"name": name, "action": action, **result.to_dict()}
    if profile.get("service_name") and context.services.host_ops.command_exists(
        "systemctl"
        ...
        result = await context.services.host_ops.run_backend(
            "systemctl", command_action, service_name, check=False
        )
        return {"name": name, "action": action, **result.to_dict()}
...
PLUGIN = PluginDefinition(
    manifest=PluginManifest(
        ...
        permissions=[
            PermissionDefinition(
                key="vpn.read", description="Читать метаданные и статус VPN-профилей."
            ),
            PermissionDefinition(
                key="vpn.write",
                description="Импортировать, удалять и управлять VPN-профилями.",
            ),
        ],
