...
async def list_proxies(
    ...
    await enforce_api_rate_limit(
        request, services, user=current_user, suffix="proxy_read"
    )
...
@router.post(
    "/proxies", response_model=ProxyResponse, status_code=status.HTTP_201_CREATED
)
async def create_proxy(
    ...
    await enforce_api_rate_limit(
        request, services, user=current_user, suffix="proxy_write"
    )
    try:
...
async def export_proxifier(
    ...
    await enforce_api_rate_limit(
        request, services, user=current_user, suffix="proxy_export"
    )
    ...
    for pid in payload.proxy_ids:
        ...
        if p:
            ...
            if p.get("password_encrypted"):
                plain_pass = services.proxy_service.decrypt_password(
...
async def check_all_proxies(
    ...
    background_tasks.add_task(
        services.proxy_service.check_all_background, current_user.user_id
    )
