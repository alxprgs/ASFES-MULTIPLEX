...
class AuditRepository:
    ...
    async def export_stream(self, start_date: str, end_date: str, **filters: Any) -> typing.AsyncGenerator[dict[str, Any], None]:
        collection = self.db.collection(AUDIT_EVENTS)
        query = self._build_query(filters)
        query["timestamp"] = {"$gte": start_date, "$lte": end_date}
        
        # hint timestamp index to prevent collection scans
        cursor = collection.find(query).sort("timestamp", DESCENDING).hint([("timestamp", DESCENDING)])
