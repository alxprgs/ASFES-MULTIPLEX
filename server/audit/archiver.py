...
class AuditArchiverJob:
    def __init__(self, db: DatabaseManager, settings: Settings, retention_days: int = 30) -> None:
        self.db = db
    ...
    async def _archive_batch(self, batch: list[dict], collection) -> None:
        ...
        result = await collection.update_many(
            ...
            {"$set": {"archived": True, "archive_file": filename, "archive_hash": archive_hash}}
        )
        
        LOGGER.info(f"Archived {result.modified_count} events to {filename} (hash: {archive_hash[:8]})")
    ...
    async def run_cleanup(self, days_old: int = 90) -> None:
        ...
        if result.deleted_count > 0:
            LOGGER.info(f"Cleaned up {result.deleted_count} archived events older than {days_old} days from MongoDB")
