...
class AsyncPypiMirror:
    ...
    def _get_pkg_dir(self, name: str) -> Path:
        # Always store with normalized (PEP 503) name
        return self._safe_path(normalize_package_name(name))

    def _get_ver_dir(self, name: str, ver: str) -> Path:
        return self._safe_path(normalize_package_name(name), ver)
    ...
    async def download_version(self, session: aiohttp.ClientSession, name: str, ver: str) -> bool:
        ...
        if ver not in releases:
            logger.error("version_not_found name=%s version=%s", norm, ver)
            return False
    ...
    async def download_all_versions(self, session: aiohttp.ClientSession, name: str) -> None:
        ...
        async def task(ver: str) -> bool:
            async with sem:
                return await self.download_version(session, norm, ver)
    ...
    async def download_version_public(
        ...
        try:
            async with self._managed_session(session) as managed:
                ok = await self.download_version(managed, norm, ver)
            path = self.get_path(norm, ver)
            result = self._result(
                ok=ok,
    ...
    async def get_package_info(
        ...
        try:
            ...
            result = self._result(False, "get_package_info", name=norm, error=str(exc))
            self._log_result(result)
            return result
    ...
    async def get_version_info(
        ...
        try:
            ...
            releases = (metadata or {}).get("releases", {})
            release_files = releases.get(ver, [])
            local_dir = self._get_ver_dir(norm, ver)
