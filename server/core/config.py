...
class AppConfig(BaseModel):
    ...
    # Enable Swagger/ReDoc UI. Defaults to True in dev mode, False in production.
    # Set APP__ENABLE_API_DOCS=true to expose docs even in production (restrict access at network level).
...
class SecurityConfig(BaseModel):
    ...
    # Separate encryption key for proxy passwords — must NOT reuse api_jwt_secret.
    # Generated by install.sh; rotate independently from JWT secrets.
    proxy_encryption_key: SecretStr = SecretStr("ChangeThisProxyEncryptionKey")
    ...
    # None = auto-detect (Secure when public_base_url uses HTTPS and not dev mode).
    # True / False overrides auto-detection explicitly.
...
class RateLimitPresetConfig(BaseModel):
    ...
    # Proxy password export: stricter limit to rate-limit credential exposure
    proxy_export_limit: int = 10
    proxy_export_window_seconds: int = 3600
...
class Settings(BaseSettings):
    ...
    def _uses_default_secret_values(self) -> bool:
        ...
        current_values = {
            ...
            self.security.proxy_encryption_key.get_secret_value(),
    ...
    def effective_cookie_secure(self) -> bool:
        """Resolve whether cookies should carry the Secure flag.

        Priority:
        1. ``allow_insecure_cookies=True`` → always False (explicit opt-out).
        2. ``cookie_secure`` is explicitly set (True/False) → use that value.
        3. Auto-detect: True when public_base_url uses HTTPS AND we are not in
           dev mode; False otherwise (local HTTP development).
        """
        if self.security.allow_insecure_cookies:
            return False
        if self.security.cookie_secure is not None:
            return self.security.cookie_secure
        # Auto-detect from the public URL scheme and dev flag
        is_https = str(self.app.public_base_url).startswith("https://")
        return is_https and not self.app.dev

    @property
    def api_docs_enabled(self) -> bool:
        """Whether Swagger/ReDoc UI endpoints are exposed.

        Defaults to True in dev mode, False in production.
        Can be overridden via APP__ENABLE_API_DOCS.
        """
        if self.app.enable_api_docs is not None:
            return self.app.enable_api_docs
        return self.app.dev
