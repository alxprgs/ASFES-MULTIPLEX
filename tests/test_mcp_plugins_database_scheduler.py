...
async def test_database_manager_plugin(host_ops, temp_workspace) -> None:
    ...
    # Save profiles
    (temp_workspace["db_profiles"] / "pg_profile.json").write_text(json.dumps(pg_profile), encoding="utf-8")
    (temp_workspace["db_profiles"] / "mysql_profile.json").write_text(json.dumps(mysql_profile), encoding="utf-8")
    ...
    mock_run = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="1", stderr=""))
    ...
    mock_run = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="Dumped", stderr=""))
    with patch.object(host_ops, "run", mock_run):
        res = await db_backup_database(context, {"profile": "pg_profile", "dump_path": "backup.sql"})
    ...
    mock_run = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="Restored", stderr=""))
    with patch.object(host_ops, "run", mock_run):
        res = await db_restore_database(context, {"profile": "mysql_profile", "dump_path": "backup.sql"})
...
async def test_scheduler_plugin_linux(host_ops) -> None:
    ...
    with patch.object(host_ops, "platform_name", "linux"):
        ...
        mock_run = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout=crontab_out, stderr=""))
        ...
        # 2. upsert_task
        mock_list = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="", stderr=""))
        mock_save = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="", stderr=""))
        with patch.object(host_ops, "run_backend", mock_list):
            with patch.object(host_ops, "run", mock_save):
                res = await sched_upsert_task(context, {
                    "name": "daily_job",
        ...
        # 3. delete_task
        mock_list = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="* * * * * # multiplex:daily_job", stderr=""))
        mock_save = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="", stderr=""))
        with patch.object(host_ops, "run_backend", mock_list):
            with patch.object(host_ops, "run", mock_save):
        ...
        # 4. run_task
        mock_list = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="* * * * * python script.py # multiplex:daily_job", stderr=""))
        mock_run_cmd = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="Script run", stderr=""))
        with patch.object(host_ops, "run_backend", mock_list):
...
async def test_scheduler_plugin_windows(host_ops) -> None:
    ...
    with patch.object(host_ops, "platform_name", "windows"):
        ...
        mock_run = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout=schtasks_csv, stderr=""))
        with patch.object(host_ops, "run_backend", mock_run):
            res = await sched_list_tasks(context, {})
            assert len(res["tasks"]) == 1
            assert res["tasks"][0]["name"] == "backup_job"
        ...
        # 2. upsert_task
        mock_save = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="SUCCESS", stderr=""))
        with patch.object(host_ops, "run", mock_save):
            res = await sched_upsert_task(context, {
                "name": "daily_job",
        ...
        # 3. delete_task
        mock_del = AsyncMock(return_value=CommandResult(command=[], returncode=0, stdout="SUCCESS", stderr=""))
        with patch.object(host_ops, "run_backend", mock_del):
            res = await sched_delete_task(context, {"name": "daily_job"})
            assert res["deleted"] is True
            mock_del.assert_called_with("schtasks", "/Delete", "/F", "/TN", "multiplex_daily_job", check=False)
