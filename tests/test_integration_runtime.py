...
@pytest.mark.asyncio
async def test_bootstrap_creates_runtime_root_plugins_and_safe_runtime_upserts(
    integration_env,
) -> None:
    ...
    recreated = await services.settings_service.set_registration(
        True, actor=actor, request_meta={"ip": "127.0.0.1", "user_agent": "pytest"}
    )
...
async def test_rest_oauth_and_mcp_flow_respects_user_scoping(integration_env) -> None:
    ...
    toggle_registration = await client.put(
    ...
    alice_login = await client.post(
        "/api/auth/login", json={"username": "alice", "password": "AlicePassword123!"}
    )
    ...
    async with mcp_gateway.lifespan():
        async with Client(mcp_transport) as mcp_client:
            ...
            connected = await client.get(
                "/api/mcp/connected-services", headers=root_headers
            )
            assert connected.status_code == 200
            service = next(
                item
                for item in connected.json()
                if item["client_id"] == oauth_client_id
            )
            ...
            assert any(
                "Tool access denied" in getattr(item, "text", "")
                for item in denied_call.content
            )
...
async def test_mcp_oauth_discovery_supports_bare_mcp_path(integration_env) -> None:
    ...
    assert (
        f"/.well-known/oauth-protected-resource{cfg.mcp_path}"
        in bare_get.headers["www-authenticate"]
    )
...
@pytest.mark.asyncio
async def test_oauth_metadata_includes_resource_scopes_and_path_aware_issuer(
    integration_env,
) -> None:
    client = integration_env["client"]
    cfg = integration_env["settings"]
    ...
    issuer_metadata = await client.get(
    ...
    resource_nested_metadata = await client.get(
...
@pytest.mark.asyncio
async def test_oauth_hardening_for_dynamic_registration_confidential_clients_and_pkce(
    integration_env,
) -> None:
    client = integration_env["client"]
    cfg = integration_env["settings"]
    ...
    dynamic_register = await client.post(
        ...
        json={
            "client_name": "Dynamic",
            "redirect_uris": ["https://example.test/dynamic"],
            "scope": "mcp",
        },
    )
    ...
    refresh_denied = await client.post(
        "/api/oauth/token",
        data={
    ...
    rotate = await client.post(
        f"/api/oauth/clients/{client_id}/secret/rotate", headers=headers
    )
...
@pytest.mark.asyncio
async def test_two_factor_protects_api_login_and_mcp_oauth_authorize(
    integration_env,
) -> None:
    client = integration_env["client"]
    cfg = integration_env["settings"]
    ...
    enable = await client.post(
        "/api/auth/2fa/enable", headers=headers, json={"code": totp_code(secret)}
    )
    ...
    oauth_denied = await client.post(
...
@pytest.mark.asyncio
async def test_admin_users_plugin_toggle_system_update_and_restart(
    ...
    disabled = await client.put(
        "/api/mcp/plugins/docker", headers=headers, json={"enabled": False}
    )
    assert disabled.status_code == 200
    assert disabled.json()["enabled"] is False

    enabled = await client.put(
        "/api/mcp/plugins/docker", headers=headers, json={"enabled": True}
    )
