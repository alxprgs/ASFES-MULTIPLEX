...
from server.mcp.plugins.firewall import (
    list_rules,
    set_enabled,
    upsert_rule,
    delete_rule,
...
async def test_firewall_plugin_linux(host_ops) -> None:
    ...
    with patch.object(host_ops, "platform_name", "linux"):
        ...
        mock_run = AsyncMock(
            return_value=CommandResult(
                command=["ufw"], returncode=0, stdout="Firewall stop/waiting", stderr=""
            )
        )
        with patch.object(host_ops, "run_backend", mock_run):
        ...
        mock_run = AsyncMock(
            return_value=CommandResult(
                command=["ufw"], returncode=0, stdout="Rule added", stderr=""
            )
        )
        with patch.object(host_ops, "run_backend", mock_run):
        ...
        mock_run = AsyncMock(
            return_value=CommandResult(
                command=["ufw"], returncode=0, stdout="Rule deleted", stderr=""
            )
        )
        with patch.object(host_ops, "run_backend", mock_run):
            res = await delete_rule(
                context,
                {"port": 80, "protocol": "tcp", "action": "allow", "direction": "in"},
            )
            assert res["deleted"] is True
            mock_run.assert_called_with(
                "ufw", "delete", "allow", "in", "80/tcp", check=False
            )
...
async def test_firewall_plugin_windows(host_ops) -> None:
    ...
    with patch.object(host_ops, "platform_name", "windows"):
        ...
        mock_run = AsyncMock(
            return_value=CommandResult(
                command=["netsh"], returncode=0, stdout="Ok.", stderr=""
            )
        )
        with patch.object(host_ops, "run_backend", mock_run):
            res = await upsert_rule(
...
async def test_vpn_plugin_lifecycle(host_ops, temp_workspace) -> None:
    ...
    imp_res = await vpn_import_profile(
        context,
        {
            "name": "office_vpn",
            "vpn_type": "openvpn",
    ...
    with patch.object(host_ops, "platform_name", "linux"):
        ...
        with patch.object(host_ops, "command_exists", mock_exists):
            ...
            # Control start
            mock_run_control = AsyncMock(
                return_value=CommandResult(
                    command=["systemctl"], returncode=0, stdout="", stderr=""
                )
            )
            with patch.object(host_ops, "run_backend", mock_run_control):
                control_res = await vpn_control(
                    context, {"name": "office_vpn", "action": "start"}
                )
...
async def test_ports_scanner_probes(host_ops) -> None:
    ...
    with patch("httpx.AsyncClient.request", mock_request):
        http_res = await probe_http(
            context, {"url": "http://127.0.0.1/health", "method": "GET"}
        )
