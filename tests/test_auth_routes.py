...
async def test_auth_routes_failures(integration_env) -> None:
    ...
    reg_disabled = await client.post(
        "/api/auth/register",
        json={"username": "bob", "password": "BobPassword123!", "email": "bob@example.com"}
    )
    ...
    bad_refresh2 = await client.post("/api/auth/refresh", json={"refresh_token": "invalid_refresh_token"})
...
async def test_auth_routes_registration_flow(integration_env) -> None:
    ...
    # Enable registration
    enable_reg = await client.put("/api/settings/registration", headers=root_headers, json={"enabled": True})
...
async def test_auth_routes_2fa_management(integration_env) -> None:
    ...
    disable_fail = await client.post(
        ...
        headers=headers,
        json={"code": "000000", "current_password": cfg.root.password.get_secret_value()}
    )
