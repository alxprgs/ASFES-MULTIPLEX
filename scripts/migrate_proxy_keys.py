...
async def migrate_proxy_keys():
    ...
    if proxy_key == "ChangeThisProxyEncryptionKey":
        print("ERROR: SECURITY__PROXY_ENCRYPTION_KEY is still set to the default value.")
        print("Please update your .env file with a secure key before running this migration.")
        sys.exit(1)
    ...
    async for proxy in collection.find({}):
        ...
        try:
            ...
            await collection.update_one(
                {"_id": proxy_id},
                {"$set": {"password_encrypted": new_encrypted}}
            )
            migrated_count += 1
